...
export default function ManagerRequestsPage() {
  ...
  const load = async () => {
    const [tenantResponse, terminationResponse] = await Promise.all([
      listUnitConnectionRequests(),
      listLeaseTerminationRequests(),
    ]);
    ...
    setError(tenantResponse.message || terminationResponse.message || '');
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-8">
      <section className="shell-card p-7 md:p-8">
        <p className="warm-badge">Quy trình vận hành</p>
        <h1 className="mt-5 font-headline text-5xl font-extrabold text-brand-ink">Yêu cầu cần xử lý</h1>
        <p className="mt-4 max-w-3xl text-lg leading-8 text-brand-muted">
          Tập trung xử lý các yêu cầu kết nối người thuê và yêu cầu chấm dứt hợp đồng cho các tài sản bạn đang được phân công.
        </p>
      </section>
          ...
          <article className="shell-card p-6" key={request.requestId}>
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex flex-wrap gap-3 lg:w-[280px] lg:flex-col">
                <Link className="btn-primary flex-1 px-4 py-3 text-sm" href={`/manager/requests/${request.requestId}/lease`}>
                  Tạo hợp đồng
                </Link>
